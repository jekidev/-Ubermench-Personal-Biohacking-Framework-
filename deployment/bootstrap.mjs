...
run('pnpm', ['install', '--frozen-lockfile'], process.env);
run('pnpm', ['typecheck']);
run('pnpm', ['test']);
run('pnpm', ['generate']);
...
if (!googleDriveConnected) {
  console.log('\nGoogle Drive is not connected. This is intentional: OAuth credentials must stay in platform connections/secrets.');
