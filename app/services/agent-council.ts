...
export function synthesizeCouncil(opinions: CouncilOpinion[]): CouncilDecision {
  ...
  const uniqueConclusions = [...new Set(opinions.map((opinion) => opinion.conclusion.trim()).filter(Boolean))]
  const disagreements = uniqueConclusions.length > 1 ? uniqueConclusions.slice(0, 8) : []
