...
import { combinedEvidenceScore } from './evidence-engine'
...
export function rankInterventions(profile: PersonalBiologyProfile, candidates: InterventionCandidate[]): InterventionCandidate[] {
  return candidates.map((candidate) => {
    const evidence = combinedEvidenceScore(candidate.evidence)
...
export function makeCandidate(name: string, evidence: EvidenceItem[] = [], personalFit = 0.5): InterventionCandidate {
  return { id: crypto.randomUUID(), name, expectedBenefits: [], risks: [], interactions: [], evidence, personalFit, priority: combinedEvidenceScore(evidence) * 0.45 + personalFit * 0.55 }
}
