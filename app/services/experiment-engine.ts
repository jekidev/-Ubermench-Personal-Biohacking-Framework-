...
export function summarizeNOf1(experiment: NOf1Experiment, nowInput: Date = new Date()) {
  ...
  const interventionStart = new Date(now); interventionStart.setDate(interventionStart.getDate() - interventionDays)
  const washoutStart = new Date(interventionStart); washoutStart.setDate(washoutStart.getDate() - washoutDays)
  const baselineStart = new Date(washoutStart); baselineStart.setDate(baselineStart.getDate() - baselineDays)
  const observations = experiment.observations.filter((item) => { const date = new Date(item.recordedAt); return Number.isFinite(item.value) && !Number.isNaN(date.getTime()) && (!experiment.metric || item.metric === experiment.metric) })
  const baseline = observations.filter((item) => { const date = new Date(item.recordedAt); return date >= baselineStart && date <= washoutStart })
  const intervention = observations.filter((item) => { const date = new Date(item.recordedAt); return date > interventionStart && date <= now })
