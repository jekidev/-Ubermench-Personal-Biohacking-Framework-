...
export function learnOutcome(
  ...
  const pooled = Math.sqrt((standardDeviation(baseline, baselineMean) ** 2 + standardDeviation(intervention, interventionMean) ** 2) / 2)
  const standardizedEffect = pooled > 0 ? delta / pooled : 0
  const sampleSize = baseline.length + intervention.length
  ...
  return { metric, baselineMean, interventionMean, delta, standardizedEffect, confidence, recommendation }
}
