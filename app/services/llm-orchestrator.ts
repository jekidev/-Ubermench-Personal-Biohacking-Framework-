...
export function saveLLMSettings(settings: LLMSettings) {
  ...
  try {
    ...
    // Storage may be unavailable/full; inference should remain usable for the current session.
  }
}
...
function normalizeUsage(usage: any) {
  ...
  return {
    inputTokens,
    outputTokens,
    totalTokens: usage.total_tokens ?? ((inputTokens ?? 0) + (outputTokens ?? 0) || undefined),
  }
}
...
async function requestAnthropic(provider: LLMProviderConfig, request: LLMRequest) {
  ...
  try {
    ...
    const text = Array.isArray(body?.content)
      ? body.content.filter((x: unknown): x is { type: string; text: string } => typeof x === 'object' && x !== null && 'type' in x && 'text' in x).filter((x) => x.type === 'text').map((x) => x.text).join('\n')
      : ''
...
async function callProvider(provider: LLMProviderConfig, request: LLMRequest) {
  ...
  if (provider.provider === 'openrouter') {
    return requestOpenAICompatible(provider.baseUrl ?? 'https://openrouter.ai/api/v1', provider.apiKey, model, request, {
      'HTTP-Referer': typeof location !== 'undefined' ? location.origin : 'https://ubermench.local',
      'X-Title': 'Uberm3nch',
    })
  }
  ...
  for (const provider of pool) {
    ...
    try {
      ...
      return {
        ...
        model: modelFor(provider) || 'configured-provider',
        text: result.text,
      ...
      errors.push(`${provider.provider}/${modelFor(provider) || 'unknown'}: ${message}`)
