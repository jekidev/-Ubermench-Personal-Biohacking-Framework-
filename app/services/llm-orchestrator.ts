...
function sanitizeForStorage(settings: LLMSettings): PersistedLLMSettings {
  return { preferFree: settings.preferFree, autoRotate: settings.autoRotate, showModel: settings.showModel, providers: settings.providers.map(({ apiKey: _apiKey, ...provider }) => provider) }
}
...
export function loadLLMSettings(): LLMSettings {
  ...
  try {
    ...
    return {
      ...structuredClone(DEFAULT_LLM_SETTINGS), ...parsed,
      providers: Array.isArray(parsed.providers)
        ? parsed.providers.map((stored) => ({
            ...
            ...stored, apiKey: sessionKeys[stored.provider],
          }))
        : structuredClone(DEFAULT_LLM_SETTINGS.providers).map((p) => ({ ...p, apiKey: sessionKeys[p.provider] })),
    }
  } catch { return structuredClone(DEFAULT_LLM_SETTINGS) }
}
...
async function requestOpenAICompatible(baseUrl: string, apiKey: string, model: string, request: LLMRequest, headers: Record<string, string> = {}) {
  ...
  try {
    const response = await fetch(`${baseUrl.replace(/\/$/, '')}/chat/completions`, { method: 'POST', headers: { Authorization: `Bearer ${apiKey}`, 'Content-Type': 'application/json', ...headers }, body: JSON.stringify({ model, messages: [...(request.system ? [{ role: 'system', content: request.system }] : []), { role: 'user', content: request.prompt }], temperature: request.temperature ?? 0.2, max_tokens: request.maxTokens ?? 1800 }), signal: timed.signal })
