...
function confidence(signals: PhenotypeAssessment[], id: FearPhenotypeId): number {
  return signals.find((signal) => signal.phenotype === id)?.confidence ?? 0;
}

export function nextBestTest(signals: PhenotypeAssessment[], events: LearningEventForPhenotype[]): TestRecommendation {
  const f3 = confidence(signals, "F3");
  const f4 = confidence(signals, "F4");
  const f5 = confidence(signals, "F5");
  const f6 = confidence(signals, "F6");
  ...
  if (f3 >= 0.6) {
    return candidate("F3_ACQUISITION", ["F3"], 0.95, "Gentagne acquisition-events peger på F3; næste test skal reproducere acquisition under en standardiseret læringsbetingelse.", ["prediction lock", "pre/post threat", "pre/post safety", "learning quality"], "high");
  }

  if (count24 > 0 && count7 < 2) {
    return candidate("F4_RETENTION_7D", ["F4"], 0.92, "24h-data findes, men 7d-datasættet er endnu utilstrækkeligt. Først skal langtidshukommelsen måles, før F4 kan styrkes.", ["7d same-context retrieval", "sleep around follow-up", "confound status"], "high");
  }

  if (f4 >= 0.3 && count7 >= 2) {
    return candidate("F4_RETENTION_7D", ["F4"], 0.88, "Der er gentagne 7d-data; en ny 7d retention måling er mest informativ for replication af F4-signalet.", ["7d same-context retrieval", "sleep", "confound status"], "high");
  }

  if (f5 >= 0.3 && stimulusData < 2) {
    return candidate("F5_STIMULUS_GENERALISATION", ["F5"], 0.84, "Der mangler tilstrækkelige stimulus-generalisation observationer; mål target → similar stimulus før en bredere konklusion.", ["same-context response", "similar-stimulus response"], "high");
  }

  if (f5 >= 0.3 && contextData < 2) {
    return candidate("F5_CONTEXT_TRANSFER", ["F5"], 0.82, "F5 er mulig; næste test skal skelne sikkerhedsretrieval i ny kontekst fra den oprindelige kontekst.", ["same-context response", "new-context response", "context similarity"], "high");
  }
  ...
  return candidate("F3_ACQUISITION", ["F3"], 0.55, "Der er endnu ikke nok data til en specifik downstream-bottleneck. Start med en standardiseret acquisition-test og låst prediction.", ["prediction lock", "pre/post threat", "pre/post safety", "learning quality"], "medium");
}
