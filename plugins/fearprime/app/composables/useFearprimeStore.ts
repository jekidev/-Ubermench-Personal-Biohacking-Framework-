...
const FollowUpInputSchema = z.object({
  ...
  similarStimulusResponse: z.number().min(0).max(100).optional(),
  newContextResponse: z.number().min(0).max(100),
  spontaneousRecovery: z.number().min(0).max(10).optional(),
  renewal: z.number().min(0).max(10).optional(),
  reinstatement: z.number().min(0).max(10).optional(),
