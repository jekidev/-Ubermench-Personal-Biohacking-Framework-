...
const candidates: ProviderCandidate[] = [
  ...
  { provider: 'anthropic', model: 'claude-a', free: false, enabled: true, hasApiKey: false },
]
...
describe('provider rotation', () => {
  ...
  it('skips disabled or keyless candidates', () => {
    expect(selectNextProvider([{ ...candidates[2], enabled: true }], { autoFreeOnly: false, providerOrder: ['anthropic'] })).toBeNull()
  })
})
