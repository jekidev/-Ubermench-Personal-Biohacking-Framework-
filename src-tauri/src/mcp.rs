...
pub struct McpApprovalRegistry {
    tokens: Mutex<HashMap<String, ApprovalRecord>>,
}
...
/// Called only after the UI's explicit human approval action. The native
/// registry creates a short-lived, single-use token bound to exact command/args.
#[tauri::command]
pub fn mcp_issue_approval(
    ...
    let now = now_ms();
    let seed = format!("{}:{}:{}", fingerprint, now, registry.tokens.lock().map_err(|_| "MCP approval registry poisoned.")?.len());
    let mut hasher = Sha256::new();
...
pub fn mcp_stdio_execute(
    ...
    validate_command(&request.command)?;
    let timeout_ms = timeout_ms(request.timeout_ms);
...
mod tests {
    ...
    fn approval_is_single_use_and_exactly_bound() {
        ...
        let record = ApprovalRecord { fingerprint: fingerprint(command, &args), expires_at_ms: now_ms() + 10_000 };
        registry.tokens.lock().unwrap().insert(token.clone(), record);
    ...
    fn rejects_exact_argument_mismatch() {
        ...
        registry.tokens.lock().unwrap().insert(
            token.clone(),
            ApprovalRecord { fingerprint: fingerprint("node", &["safe.js".into()]), expires_at_ms: now_ms() + 10_000 },
        );
